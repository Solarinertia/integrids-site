  ...
  return (
    <div className={overlayClass} onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
      <div className={boxClass}>
        ...
        <form className={styles.quoteForm} onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }}>
          ...
          <div className={styles.quoteActions}>
            <button type="submit" className={styles.quoteSubmitBtn} disabled={!canSubmit}>Request Quote</button>
          </div>
        </form>
      </div>
    </div>
...
export default function StorePage() {
  ...
  // On mount: read hash, expand View More if the target card is hidden, or scroll immediately
  useEffect(() => {
    ...
    if (hiddenIds.includes(hash)) {
      pendingScrollId.current = hash;
      setShowServices(true);
    } else {
      // Card already visible — wait one frame for paint then scroll
      setTimeout(() => {
  ...
  // After showServices state flips to true (and DOM updates), scroll to the pending card
  useEffect(() => {
    if (!showServices || !pendingScrollId.current) return;
    const targetId = pendingScrollId.current;
    pendingScrollId.current = null;
    // setTimeout gives React time to render the newly visible cards into the DOM
    setTimeout(() => {
