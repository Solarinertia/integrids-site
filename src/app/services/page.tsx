...
function ServiceQuoteModal({ serviceName, onClose }: { serviceName: string; onClose: () => void }) {
  ...
  async function handleSubmit() {
    ...
    try {
      await submitEnquiry("Service Request", form);
      setSubmitted(true);
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : "We could not send your service request right now. Please try again.");
    } finally {
      setSubmitting(false);
    }
  }
          ...
          <>
            ...
            <div className={styles.sqForm}>
              ...
              <div className={styles.sqActions}>
                ...
                <button className={styles.sqSubmitBtn} disabled={!canSubmit || submitting} onClick={handleSubmit}>
